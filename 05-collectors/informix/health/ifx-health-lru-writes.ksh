...
# Purpose : Normalize and validate IBM Informix LRU writes source output.
#
# Change Control
# Date    : 2026-09-14
# Author  : Norba
...
#
# Output:
#   Non-negative integer LRU write counter.
#
# Exit codes:
...
if [ -z "$VALUE" ]; then
    print -u2 "empty LRU writes value"
    exit 1
fi
...
if [ "$VALUE_COUNT" -ne 1 ]; then
    print -u2 "multiple LRU writes values"
    exit 1
fi
...
case "$VALUE" in
    *[!0-9]*)
        print -u2 "invalid LRU writes value"
        exit 1
        ;;
esac
